...
export const transformRegistry: TransformRegistry = {
	...
	[TransformType.CONDITIONAL_ON]: (value, params, context) => {
		const { checkField, if: condition, then: thenOps, else: elseOps = [] } = params || {}
		if(!condition) {
			throw new Error('conditionalOn requires "if" parameter')
		}

		if(!checkField) {
			throw new Error('conditionalOn requires "checkField" parameter')
		}

		if(!context || context[checkField] === undefined) {
			throw new Error(`Field '${checkField}' not found in context`)
		}

		// Evaluate condition on the specified field from context
		const valueToCheck = context[checkField]

		return evaluateCondition(valueToCheck, condition) ? thenOps : elseOps
	},
