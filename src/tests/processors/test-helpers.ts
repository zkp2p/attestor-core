...
import { Processor, TransformOperation, TransformType } from 'src/types/processor'
	...
	if(processor.transform) {
		for(const [varName, rule] of Object.entries(processor.transform)) {
			...
			for(const op of rule.ops) {
				value = applyOperation(value, op, context)
			}
...
function applyOperation(value: any, op: TransformOperation | string, context?: Record<string, any>): any {
	let opName: string
	let params: any = {}
	...
	if(opName === TransformType.CONDITIONAL_ON) {
		...
		for(const subOp of operations) {
			...
			if(subOpName === TransformType.CONDITIONAL_ON) {
		...
		let result = value
		for(const subOp of operations) {
			result = applyOperation(result, subOp, context)
		}
